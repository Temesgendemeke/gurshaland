...
import { Apple, BookOpen, ChefHat, Download, NotebookPen, SaveIcon, ShoppingCart, Sparkles, X, Zap } from "lucide-react"
        ...
        <Dialog >
            ...
            <DialogContent className="w-full max-w-[95vw] sm:max-w-7xl overflow-y-auto max-h-[calc(100vh-4rem)] sm:max-h-[calc(100vh-8rem)] p-0 m-0">
                <div className="relative">
                    <DialogHeader className="sticky top-0 left-0 z-50 bg-white/60 dark:bg-gray-900/60 backdrop-blur-xl w-full p-3 sm:p-5">
                        <DialogTitle>Personalized Meal Plan</DialogTitle>
                        <DialogDescription>
                            Your custom weekly meal plan, crafted to match your goals, diet preferences, and calorie targets.
                        </DialogDescription>
                    </DialogHeader>
                    <div className="flex flex-col lg:flex-row gap-4 p-2 sm:p-4">
                                    ...
                                    {plan.days.map((d, dayIdx) => (
                                        <Card
                                            ...
                                            className={`
                                                ...
                                                ${plan.days.length % 2 !== 0 && dayIdx === plan.days.length - 1
                        ...
                        <div className="w-full lg:w-80 space-y-4 sm:space-y-6">
                            ...
                            {/* Tips Card */}
                            {plan.pro_tips && plan.pro_tips.length > 0 && <Card className="border-2 shadow-xl bg-gradient-to-br from-emerald-50 to-sky-50 dark:from-gray-900 dark:to-gray-900">
                                <CardHeader className="pb-4">
                                    <CardTitle className="text-xl font-bold flex items-center gap-2">
                                ...
                                <CardContent className="space-y-3 max-h-[calc(100vh-39rem)] overflow-y-auto">
                                    {plan.pro_tips.map((tip, idx) => (
                                        <div key={idx} className="flex items-start gap-3 p-3 rounded-lg bg-white/60 dark:bg-gray-800/60">
                                            <NotebookPen className="h-5 w-5 text-emerald-600 shrink-0 mt-0.5" />
                                            <div className="text-sm"><ReactMarkdown>{tip}</ReactMarkdown> </div>
                                        </div>
                                    ))}
                                </CardContent>
                            </Card>}
                    ...
                    <DialogFooter className="sticky bottom-0 left-0 z-50 bg-white/60 dark:bg-gray-900/60 backdrop-blur-xl w-full p-3 sm:p-4 flex-col-reverse sm:flex-row gap-2">
                        <DialogClose asChild>
                        ...
                        <Button type="button" className="btn-secondary-modern w-full sm:w-auto">
                            <Download className="h-4 w-4 sm:h-5 sm:w-5 mr-2" />
                            <span>Download PDF</span>
                        </Button>
                        <Button type="submit" className="btn-primary-modern w-full sm:w-auto" onClick={handleSave}>
